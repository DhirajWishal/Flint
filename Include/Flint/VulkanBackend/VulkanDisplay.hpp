...
namespace Flint
{
	namespace VulkanBackend
	{
		...
		{
		public:
			VulkanDisplay(Instance &instance, const FExtent2D &extent, const std::string &title) : Display(instance, extent, title)
			{
			...
			virtual UI32 FindBestBufferCount(Device &device, UI32 count = 0) override final
			{
				...
				else if (count == 0)
				{
					...
					if (vSurfaceCapabilities.maxImageCount > 0 && bufferCount > vSurfaceCapabilities.maxImageCount)
		...
		public:
			void CallKeyCallback(I32 scancode, I32 action, I32 mods)
			{
				if (pKeyCallback)
					pKeyCallback(mKeyMap[scancode], mActionMap[action], static_cast<SpecialCharacter>(mods));
			}

			void CallMouseButtonCallback(I32 button, I32 action, I32 mods)
			{
				if (pMouseButtonCallback)
					pMouseButtonCallback(mButtonMap[button], mActionMap[action], static_cast<SpecialCharacter>(mods));
			}

			void CallCursorPositionCallback(double xOffset, double yOffset)
			{
				if (pCursorPositionCallback)
					pCursorPositionCallback(xOffset, yOffset);
			}

			void CallMouseScrollCallback(double xOffset, double yOffset)
			{
				if (pMouseScrollCallback)
					pMouseScrollCallback(xOffset, yOffset);
			}

			void CallCursorWithinDisplayCallback(bool state)
			{
				if (pCursorWithinDisplayCallback)
					pCursorWithinDisplayCallback(state);
			}

			void CallDragAndDropCallback(std::vector<std::filesystem::path> paths)
			{
				if (pDragAndDropCallback)
					pDragAndDropCallback(paths);
			}

			void CallDisplayResizeCallback(FExtent2D extent)
			{
				if (pDisplayResizeCallback)
					pDisplayResizeCallback(extent);
			}

			void CallDisplayCloseCallback()
			{
				if (pDisplayCloseCallback)
					pDisplayCloseCallback();
			}
