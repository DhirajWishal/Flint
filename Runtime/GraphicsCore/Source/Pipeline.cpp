...
namespace Flint
{
	void Pipeline::WriteDataToCacheFile(const uint64_t size, unsigned char* pData) const
	{
	...
	std::pair<uint64_t, unsigned char*> Pipeline::ReadDataFromCacheFile() const
	{
		// Return if the name is empty.
		if (mPipelineName.empty())
	...
	std::shared_ptr<ResourcePackage> Pipeline::CreateResourcePackage(const uint64_t index)
	{
