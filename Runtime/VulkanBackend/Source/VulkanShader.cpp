...
namespace Flint
{
	namespace VulkanBackend
	{
		namespace Helpers
		{
			std::vector<uint32_t> ResolvePadding(const std::vector<uint32_t>& code)
			{
				const uint64_t finalCodeSize = code.size() / 4;
				std::vector<uint32_t> resolvedCode(code.begin(), code.begin() + finalCodeSize);
		...
		void VulkanShader::Reload(const std::vector<uint32_t>& code)
		{
			OPTICK_EVENT();
			Terminate();
			bIsTerminated = false;
