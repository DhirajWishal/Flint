...
namespace Flint
{
	...
	/**
	 * Off screen result layer mode enum.
	 * This defines how the layers are mapped to a frame.
	 */
	enum class OffScreenResultLayerMode : UI8 {
		/**
		 * Default setting.
		 * This means that layers are not used, instead a single main result image (with layers) will be used.
		 */
		DEFAULT,

		/**
		 * Apply one layer for each frame. If the layer count outmatches the frame count, the frame buffer will contain clusters of frames.
		 * Each cluster will contain frames with the same count as layers, and there will be one or more clusters as specified by the user.
		 */
		 PER_FRAME,
	};

	/**
	 * Off screen result specification structure.
	 */
	struct OffScreenResultSpecification
	{
		OffScreenResultSpecification() = default;
		OffScreenResultSpecification(OffScreenRenderTargetAttachment attachment, UI32 layers = 1, PixelFormat format = PixelFormat::R8G8B8A8_SRGB, OffScreenResultLayerMode mode = OffScreenResultLayerMode::DEFAULT)
			: mAttachment(attachment), mLayerCount(layers), mPixelFormat(format), mLayerMode(mode) {}

		UI32 mLayerCount = 1;
		OffScreenRenderTargetAttachment mAttachment = OffScreenRenderTargetAttachment::COLOR_BUFFER;
		OffScreenResultLayerMode mLayerMode = OffScreenResultLayerMode::DEFAULT;
		PixelFormat mPixelFormat = PixelFormat::R8G8B8A8_SRGB;
	};
