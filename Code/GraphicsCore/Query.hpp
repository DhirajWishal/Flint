...
namespace Flint
{
	...
	constexpr bool operator&(const QueryDataMode lhs, const QueryDataMode rhs) { return static_cast<UI8>(lhs) & static_cast<UI8>(rhs); }
	constexpr QueryDataMode operator|(const QueryDataMode lhs, const QueryDataMode rhs) { return static_cast<QueryDataMode>(static_cast<UI8>(lhs) | static_cast<UI8>(rhs)); }
	...
	public:
		 ...
		 */
		Query(const std::shared_ptr<Device>& pDevice, const QueryUsage usage, const UI32 queryCount) : DeviceBoundObject(pDevice), mUsage(usage), mQueryCount(queryCount) {}
		 ...
		 */
		virtual void Recreate(const UI32 queryCount) = 0;
		 ...
		 */
		virtual void RequestQueryData(const UI32 firstQuery, const UI32 count, const UI64 dataSize, void* pDataStore, const UI64 stride, const QueryDataMode dataMode) = 0;
	...
	public:
		 ...
		 */
		UI32 GetQueryCount() const { return mQueryCount; }
