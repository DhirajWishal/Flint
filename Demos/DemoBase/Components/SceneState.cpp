...
{
	mSolutionPath = boost::filesystem::path(__FILE__).parent_path().parent_path().parent_path().parent_path();
	...
	pVertexShader = pDevice->CreateShader(Flint::ShaderType::VERTEX, boost::filesystem::path("Flint\\Shaders\\3D\\shader.vert.spv"));
	pFragmentShader = pDevice->CreateShader(Flint::ShaderType::FRAGMENT, boost::filesystem::path("Flint\\Shaders\\3D\\shader.frag.spv"));
...
{
	pVertexShaderBB = pDevice->CreateShader(Flint::ShaderType::VERTEX, boost::filesystem::path("Flint\\Shaders\\BoundingBox\\shader.vert.spv"));
	pFragmentShaderBB = pDevice->CreateShader(Flint::ShaderType::FRAGMENT, boost::filesystem::path("Flint\\Shaders\\BoundingBox\\shader.frag.spv"));
