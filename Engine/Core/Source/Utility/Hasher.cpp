...
namespace Flint
{
	uint64_t GenerateHash(const Core::ResourceBindingTable& table)
	{
	...
	uint64_t GenerateHash(const Core::RasterizingPipelineSpecification& specification)
	{
		...
		const XXH64_hash_t hashes[] = {
			...
			XXH64(reinterpret_cast<const std::byte*>(&specification) + offsetof(Core::RasterizingPipelineSpecification, m_ColorBlendConstants), sizeof(Core::RasterizingPipelineSpecification) - offsetof(Core::RasterizingPipelineSpecification, m_ColorBlendConstants), 0),
		};

		return static_cast<uint64_t>(XXH64(hashes, sizeof(hashes), 0));
	}

	uint64_t GenerateHash(const Core::VertexDescriptor& descriptor)
	{
		return static_cast<uint64_t>(XXH64(descriptor.data(), sizeof(Core::DataType) * descriptor.size(), 0));
	}
}