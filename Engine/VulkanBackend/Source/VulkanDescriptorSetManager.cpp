...
namespace Flint
{
	...
	{
		VulkanDescriptorSetManager::VulkanDescriptorSetManager(VulkanEngine& engine, uint8_t frameCount)
			: m_Engine(engine), m_FrameCount(frameCount)
		{
		}
		...
		void VulkanDescriptorSetManager::registerTable(const ResourceBindingTable& table)
		{
			...
			FLINT_VK_ASSERT(m_Engine.getDeviceTable().vkAllocateDescriptorSets(m_Engine.getLogicalDevice(), &allocateInfo, descriptorSets.data()), "Failed to allocate descriptor set!");
			...
			for (uint8_t i = 1; i < m_FrameCount; i++)
			{
				...
				m_Engine.getDeviceTable().vkUpdateDescriptorSets(m_Engine.getLogicalDevice(), 0, nullptr, static_cast<uint32_t>(copyDescriptorSets.size()), copyDescriptorSets.data());
			}
