...
namespace Flint
{
	...
	public:
		FImage(FDevice* pDevice, UI64 width, UI64 height, UI64 depth, ImageUsage usage, UI8 bitsPerPixel = FLINT_DEFAULT_BPP, UI8 layers = 1)
...
{
	template<>
	struct hash<Flint::SamplerSpecification> {
		const size_t operator()(const Flint::SamplerSpecification& spec) const
		{
			return Flint::Hasher::QuickHash64(&spec, sizeof(Flint::SamplerSpecification));
		}
	};
}