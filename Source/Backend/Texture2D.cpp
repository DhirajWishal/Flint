...
#include "Flint/Backend/Texture2D.hpp"
...
namespace Flint
{
	namespace Backend
	{
		...
		std::shared_ptr<Flint::Backend::Texture2D> Texture2D::LoadFromFile(const std::shared_ptr<Device>& pDevice, const std::filesystem::path& assetFile, ImageUsage usage, Multisample multisampleCount /*= Multisample::One*/)
		{
			OPTICK_EVENT();
